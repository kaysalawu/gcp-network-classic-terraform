...
locals {
  vpc_sc_config_case1_create = templatefile("../../init/vpc-sc/create.sh", {
    ORGANIZATION_ID = var.organization_id
    POLICY_TITLE    = "${local.prefix}-policy"
    ACCESS_LEVELS   = [local.case1_access_level]
    PERIMETERS      = local.case1_perimeters
  })
  vpc_sc_config_case1_delete = templatefile("../../init/vpc-sc/delete.sh", {
    ORGANIZATION_ID = var.organization_id
    POLICY_TITLE    = "${local.prefix}-policy"
    ACCESS_LEVELS   = [local.case1_access_level]
    PERIMETERS      = local.case1_perimeters
...
locals {
  vpc_sc_config_case2_create = templatefile("../../init/vpc-sc/create.sh", {
    ORGANIZATION_ID = var.organization_id
    POLICY_TITLE    = "${local.prefix}-policy"
    ACCESS_LEVELS   = local.case2_access_levels
    PERIMETERS      = local.case2_perimeters
  })
  vpc_sc_config_case2_delete = templatefile("../../init/vpc-sc/delete.sh", {
    ORGANIZATION_ID = var.organization_id
    POLICY_TITLE    = "${local.prefix}-policy"
    ACCESS_LEVELS   = local.case2_access_levels
    PERIMETERS      = local.case2_perimeters
