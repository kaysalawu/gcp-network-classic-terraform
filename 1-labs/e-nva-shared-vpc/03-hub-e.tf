...
resource "google_compute_subnetwork" "hub_int_subnets" {
  for_each = merge(
    { for k, v in local.hub_int_subnets : k => v if v.purpose != "REGIONAL_MANAGED_PROXY" },
  )
  provider      = google-beta
  project       = var.project_id_hub
  name          = each.key
...
resource "google_compute_subnetwork" "spoke1_subnets" {
  for_each      = { for k, v in local.spoke1_subnets : k => v if v.purpose != "REGIONAL_MANAGED_PROXY" }
  provider      = google-beta
  project       = var.project_id_hub
  name          = each.key
  network       = google_compute_network.hub_int_vpc.id
...
resource "google_compute_subnetwork" "spoke2_subnets" {
  for_each      = { for k, v in local.spoke2_subnets : k => v if v.purpose != "REGIONAL_MANAGED_PROXY" }
  provider      = google-beta
  project       = var.project_id_hub
  name          = each.key
  network       = google_compute_network.hub_int_vpc.id
...
locals {
  hub_files = {
    "output/hub-eu-nva.sh"  = local.hub_nva_eu_startup
    "output/hub-us-nva.sh"  = local.hub_nva_us_startup
