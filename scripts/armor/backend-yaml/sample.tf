...
locals {
  ...
  hub_xlb7_backend_sec_yaml = templatefile("../../init/armor/policy/policy.yaml", {
    PROJECT_ID  = var.project_id_hub
    POLICY_NAME = local.hub_xlb7_backend_sec_policy
    RULES       = local.hub_xlb7_sec_rules
    TYPE        = "CLOUD_ARMOR"
  })
  hub_xlb7_backend_sec_rules_create = templatefile("../../init/armor/policy/create.sh", {
    PROJECT_ID       = var.project_id_hub
    POLICY_NAME      = local.hub_xlb7_backend_sec_policy
  ...
  hub_xlb7_backend_sec_rules_delete = templatefile("../../init/armor/policy/delete.sh", {
    PROJECT_ID       = var.project_id_hub
    POLICY_NAME      = local.hub_xlb7_backend_sec_policy
    POLICY_FILE_NAME = "${local.hub_prefix}owasp-backend.yaml"
    POLICY_FILE_DIR  = "output/armor"
...
locals {
  ...
  hub_xlb7_edge_sec_yaml = templatefile("../../init/armor/policy/policy.yaml", {
    PROJECT_ID  = var.project_id_hub
    POLICY_NAME = local.hub_xlb7_edge_sec_policy
    RULES       = local.hub_xlb7_sec_rules
    TYPE        = "CLOUD_ARMOR_EDGE"
  })
  hub_xlb7_edge_sec_rules_create = templatefile("../../init/armor/policy/create.sh", {
    PROJECT_ID       = var.project_id_hub
    POLICY_NAME      = local.hub_xlb7_edge_sec_policy
  ...
  hub_xlb7_edge_sec_rules_delete = templatefile("../../init/armor/policy/delete.sh", {
    PROJECT_ID       = var.project_id_hub
    POLICY_NAME      = local.hub_xlb7_edge_sec_policy
    POLICY_FILE_NAME = "${local.hub_prefix}owasp-edge.yaml"
    POLICY_FILE_DIR  = "output/armor"
